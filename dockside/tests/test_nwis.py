...
@pytest.mark.parametrize(('daily', 'expected'), [
...
def test_daily_json(fetch, station):
    ...
    fetch.assert_called_once_with(station.site, station.start,
                                  station.end, daily=True)
...
@pytest.mark.parametrize(('daily', 'fname'), [
    (True, '14211500_20181001_thru_20181030_daily.csv'),
    (False, '14211500_20181001_thru_20181030_insta.csv')
])
...
def test_station_get_data(nwis_reader, cache_reader, station, daily, fname, save, force, exists):
    ...
    with patch('pathlib.Path.exists', return_value=exists):
        with patch('pandas.DataFrame.to_csv', return_value=None) as to_csv:
            ...
            if not exists or force:
                nwis_reader.assert_called_once_with(station.site, station.start, station.end, daily=daily)
                if save:
